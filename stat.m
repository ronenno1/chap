classdef stat
    methods(Static)
        ...
        function plot_statistics(src, evnt, fig, figure, statistical_data, total_data, contrast_id, action, next, back)
            ...
            contrast_data  = statistical_data.contrasts.(approach_str).(char(contrast_names(contrast_id+action)));
...
%                 grouper.do_plot(total_data, total_data.configuration, total_data.configuration.rate, fig);
            ...
            ylabel(fig, 't-value', 'FontWeight','bold');
            if bayesian
            ...
            if strcmp(statistical_data.configuration.vals.output_type, 'descriptive')
        ...
        function [t, bf, N, sd, pes] = ttest_and_bf(data1, data2)
    ...
    %                         ['samples analyzed: ' num2str(percentage) '%']
                            percentages(percentage/10) = 1;
                        end;

                        data1 = full_data1(:, sample);
                        data2 = full_data2(:, sample);
                        [t, bf, ~, ~, pes] = stat.ttest_and_bf(data1, data2);
