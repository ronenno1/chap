function output = als2matlab(full_als_name, output_folder_name, log, events2, vars2)
    ...
    output = [];
    [path, file_name, ext] = fileparts(full_als_name);
    if ~strcmpi(ext, '.als')
        return;
    end;
    ...
    print_log(['Start load and convert ALS file: ' strrep(file_name, '_', '\_') ext], log);

        
    print_log(['Start loading and convert csv file: ' strrep(file_name, '_', '\_') ext], log);
    copyfile(full_als_name,[path filesep file_name '.dat']);
